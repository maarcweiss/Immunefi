...
contract Signers {
    ...
    function _recoverSigner(
        ...
        //@audit doesn't it return 0??
        //@audit it does revert on invalid sig but not if it returns a random address
    ...
    function removeSigner(
        ...
        //@audit you can remove signers multichain signature replay
        //@audit if someone removes/add a signer in a chain you can replay it in another chain if the contract address is the same
        //@audit no chainId , so if the contract address are the same,which is very commone in multichain contracts, you can remove signers getting the signature
        //@audit false they asre using chainId in verufy signers
        bytes32 _hash = keccak256(
            abi.encodePacked(
                nonce,
                address(this),
